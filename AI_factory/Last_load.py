...
def attention_gate(g, s, num_filters):
    # Resize s tensor to match the size of out tensor
    s = L.Conv2D(num_filters, 1, padding="same")(s)
    s = L.BatchNormalization()(s)
    
    # Convolution layers for g tensor
    Wg = L.Conv2D(num_filters, 1, padding="same")(g)
    Wg = L.BatchNormalization()(Wg)

    # Combine Wg and resized s tensors
    combined = L.Add()([s, Wg])
    combined = L.Activation("swish")(combined)
    
    # Convolution layers for combined tensor
    out = L.Conv2D(num_filters, 1, padding="same")(combined)
    out = L.Activation("sigmoid")(out)

    # Apply attention mechanism
    gated_s = L.Multiply()([out, s])
...
BATCH_SIZE = 16 # batch size 지정               더 줄여 볼 것
...
# 최종 가중치 저장 이름
FINAL_WEIGHTS_OUTPUT = 'model_{}_{}_final_weights2.h5'.format(MODEL_NAME, save_name)
...
def get_model(model_name, nClasses=1, input_height=128, input_width=128, n_filters = 16, dropout = 0.1, batchnorm = True, n_channels=N_CHANNELS):
...
model.compile(optimizer = Adam(learning_rate= 0.000000001), loss = sm.losses.bce_jaccard_loss, metrics = ['accuracy', miou])
model.summary()
model.load_weights('C:/dataset/train_output/checkpoint-pretrained_attention_unet-base_line-epoch_200.hdf5', )


# checkpoint 및 조기종료 설정
es = EarlyStopping(monitor='val_miou', mode='max', verbose=1, patience = 5  , restore_best_weights=True)
...
# model = get_model(MODEL_NAME, input_height=IMAGE_SIZE[0], input_width=IMAGE_SIZE[1], n_filters=N_FILTERS, n_channels=N_CHANNELS)
# model.compile(optimizer = Adam(learning_rate= 0.005), loss = sm.losses.bce_jaccard_loss, metrics = ['accuracy', miou])

# model.summary()
# model.load_weights('C:\dataset/train_output/model_unet_base_line_final_weights2.h5')
# model = load_model('C:\dataset\train_output\checkpoint-pretrained_attention_unet-base_line-epoch_175.hdf5')

# model = load_model('C:/dataset/train_output/checkpoint-pretrained_attention_unet-base_line-epoch_200.hdf5', 
#                    custom_objects={'binary_crossentropy_plus_jaccard_loss': sm.losses.bce_jaccard_loss ,'miou' : miou  }
                #    )

# C:\dataset\train_output\checkpoint-pretrained_attention_unet-base_line-epoch_200.hdf5
...
for i in test_meta['test_img']:
    ...
    y_pred = np.where(y_pred[0, :, :, 0] > 0.28 , 1 , 0) # 임계값 처리
