...
def ohem_loss(y_true, y_pred, ratio=3):
    # 이진 교차 엔트로피 손실 계산
    bce_loss = tf.keras.losses.binary_crossentropy(y_true, y_pred)
    
    # 어려운 예제 식별을 위해 손실 값을 정렬
    sorted_losses, _ = tf.nn.top_k(tf.reshape(bce_loss, [-1]), k=tf.shape(bce_loss)[0] // ratio)
    
    # 어려운 예제에 대한 손실 평균 계산
    ohem_loss = tf.reduce_mean(sorted_losses)
...
model.summary()
model.load_weights('C:\dataset/train_output/model_pretrained_attention_unet_base_line_final_weights2_last.h5')
# model = load_model('C:\dataset/train_output/checkpoint-pretrained_attention_unet-base_line-epoch_175.hdf5')

# checkpoint-pretrained_attention_unet-base_line-epoch_200.hdf5
