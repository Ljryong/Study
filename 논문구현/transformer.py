...
model.fit(
    ...
    epochs=1000,
    batch_size=32,
    callbacks = [EarlyStopping(monitor='loss', patience= 70, mode= 'min', verbose= 1, restore_best_weights=True)]
)
...
# 번역 결과 출력
for source, decoded_sequence in zip(source_sentences, decoded):
    decoded_sentence = ''.join([target_token_dict_inv.get(token, '') for token in decoded_sequence if token > 2])  # 특수 토큰 제외하고 출력
    print(f"번역 전:\n{source}\n번역 후:\n{decoded_sentence}")
