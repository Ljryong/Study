...
#2. 모델구성
# Layer 1 
x = tf.compat.v1.placeholder(tf.float32 , shape=[None,32,32,3] )        # None 으로 두는 이유는 batch 로 자를수도 있고 다른 여러가지 이유로 None 으로 둠
y = tf.compat.v1.placeholder(tf.float32 , shape=[None,10] )
drop = tf.compat.v1.placeholder(tf.float32 )

w1 = tf.compat.v1.get_variable('w1', shape=[2,2,3,32] )     # shape=[커널사이즈(2,2), 컬러(채널)(1) , 필터(아웃풋)(64)]
...
# Layer4 DNN
w4 = tf.compat.v1.get_variable('w4' , shape=[7*7*16 , 100] )
...
# Layer5 DNN
w5 = tf.compat.v1.get_variable('w5' , shape=[100, 100] )
