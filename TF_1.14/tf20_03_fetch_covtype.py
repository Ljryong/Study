...
w1 = tf.compat.v1.Variable(tf.random_normal([54,30]) , name = 'weight1' )
b1 = tf.compat.v1.Variable(tf.zeros([30]) , name = 'bias1' )                    # 더 해지는 것이라서 행을 맞추지 않고 열을 아웃풋과 맞춰야 됨
layer1 = tf.nn.softmax(tf.compat.v1.matmul(xp,w1) + b1 )

w2 = tf.compat.v1.Variable(tf.random_normal([30,10]) , name = 'weight2' )
b2 = tf.compat.v1.Variable(tf.zeros([10]) , name = 'bias2' )              
layer2 = tf.compat.v1.matmul(layer1,w2) + b2 

w3 = tf.compat.v1.Variable(tf.random_normal([10,7]) , name = 'weight3' )
