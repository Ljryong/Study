...
train1 = train1.drop(['외인비'],axis=1).drop(['신용비'],axis=1).drop(['전일비'],axis=1)
train2 = train2.drop(['외인비'],axis=1).drop(['신용비'],axis=1).drop(['전일비'],axis=1)
...
x1_train , x1_test , x2_train , x2_test , y1_train , y1_test ,y2_train,y2_test = train_test_split(x1,x2,y1,y2,
                                                                                  test_size=0.3 , random_state= 22011 , shuffle=True )
...
x1_train = x1_train.reshape(-1,65)
x2_train = x2_train.reshape(-1,65)
x1_test = x1_test.reshape(-1,65)
x2_test = x2_test.reshape(-1,65)
...
x1_train = x1_train.reshape(-1,5,13)
x2_train = x2_train.reshape(-1,5,13)
x1_test = x1_test.reshape(-1,5,13)
x2_test = x2_test.reshape(-1,5,13)
...
model = load_model('c:/_data/sihum/sihum_s10_a12.h5',compile=True)
