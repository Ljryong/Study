...
# mcp = ModelCheckpoint(monitor='val_loss', mode='min' , verbose=1, save_best_only=True , filepath=  'c:/_data/_save/MCP/keras26_MCP7.hdf5'   )

# #3 컴파일, 훈련
# model.compile(loss = 'binary_crossentropy' , optimizer= 'adam' , metrics=['acc'])
# model.fit(x_train,y_train,epochs = 1000000 , batch_size = 5 , validation_split=0.2 , callbacks = [es,mcp] )

model = load_model('c:/_data/_save/MCP/keras26_MCP7.hdf5')
