...
x_train = np.array([1,2,3,4,5])         # 데이터를 30% 손해를 보더라도 해야되는 이유는 과접합을 조심하기 위해서이다.
y_train = np.array([1,2,3,4,6])         # 실무에서 보통 7 : 3 으로 나눈다. 7은 훈련 3은 평가로 사용한다. 
...
model.fit(x_train,y_train,epochs=1000 , batch_size= 5, validation_data = (x_val,y_val))
