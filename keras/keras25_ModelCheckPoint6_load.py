...
# hist = model.fit(x_train,y_train, epochs = 1000 , batch_size= 10 , validation_split=0.2 , callbacks = [es, mcp] , verbose = 1 )
...
model = load_model('../_data/_save/MCP/k25_0117_1204_0024-26.1886.hdf5')
...
print('==============================')
# print(hist.history['val_loss'])           # val_loss 는 뽑을 수 없다.
print('==============================')

# restore_best_weights
# save_best_only

# True , True 일때  밀린뒤가 없다.  / 체크 포인트들만 저장 / 제일 좋다?
# True , False 일때 밀린값까지 저장 / 모델 훈련된게 훈련이 멈춘 마지막 순간 ,밀린게 없어도 모델은 
# False , True 일때 밀린뒤로는 없다 /  체크포인트들을 전부 저장
# False , False 일때 es 안에 모든 값을 저장됨 / 모델 훈련된게 훈련이 멈춘 마지막 순간
