...
video = np.random.rand(100, 64, 64, 3)  # 예시 비디오 데이터

fig = plt.figure()

# 초기 프레임을 플롯합니다.
im = plt.imshow(plot_video[0])

# 애니메이션 갱신 함수를 정의합니다.
def updatefig(j):
    im.set_array(plot_video[j])
    return [im]

# 애니메이션을 생성합니다.
ani = animation.FuncAnimation(fig, updatefig, frames=range(len(plot_video)), interval=50, blit=True)

ed_t = time.time()
print('걸린시간 :', round(ed_t - st_t,2), '초')
