...
# Import the MoViNet model from TensorFlow Models (tf-models-official) for the MoViNet mod777881el
...
# 모델에 가중치 로드
#model.load_weights("C:\prodownload/movinet_gazua_w2.h5")
...
print("가중치 저장!!!!")
model.save_weights("C:\prodownload/movinet_final_w.h5")
model.save("C:\prodownload/movinet_final_m.h5")
print("가중치 저장!!!!")
...
import json
with open('C:\prodownload\\token_dic.json', 'r', encoding = 'utf-8') as json_file:          ### 딕셔너리 불러오기
...
def find_first_video_files(folder_path):
    ...
    for subfolder in base_path.iterdir():
        if subfolder.is_dir():
            ...
            if video_files:
                video_paths.append(str(video_files[0]))
    return video_paths
...
play_videos_with_korean_labels(video_paths, actual_labels, predicted_labels, font_path) 
    ...
    for index, video_path in enumerate(video_paths):
        ...
        while cap.isOpened():
            ...
            if ret:
                ...
                # 텍스트 그리기 함수를 호출하기 전에 프레임 크기 조정 필요
                frame = draw_text_on_frame(frame, label_text, (10, 30), font_path, 30, (255, 255, 255))
