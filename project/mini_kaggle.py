...
NUM_THREADS = 22  # 사용하고 싶은 CPU 쓰레드 수
...
CLASSES = sorted(os.listdir('C:/prodownload/train'))
...
checkpoint_dir = 'D:\\movinet_a3_stream'
...
CLASSES = ['Bye', 'Hello', 'House', 'Nice', 'No', 'No action made', 'Thank you', 'Yes']
...
def to_gif(images,rand_int):
  converted_images = np.clip(images * 255, 0, 255).astype(np.uint8)
  imageio.mimsave(f'./animation_new{rand_int}.gif', converted_images, fps=10)
  print(rand_int)
  return embed.embed_file(f'./animation_new{rand_int}.gif')
import random
# !rm *.gif
# Create initial states for the stream model
init_states_fn = model.init_states
init_states = init_states_fn(tf.shape(tf.ones(shape=[1, 1, 256, 256, 3])))
...
to_gif(frames[0].numpy(),np.random.randint(1,100))
'''